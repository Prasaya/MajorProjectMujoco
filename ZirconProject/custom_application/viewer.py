...
class Viewer:
  ...
  def __init__(self, viewport, mouse, keyboard, camera_settings=None,
               zoom_factor=_FULL_SCENE_ZOOM_FACTOR, scene_callback=None, pause_subject = None):
    ...
    self._camera_paths = list()
    self._pause_subject = pause_subject
  ...
  def initialize(self, physics, renderer_instance, touchpad):
    ...
    self._camera.set_tracking_mode(2)
  ...
  def render(self):
    ...
    if self._camera and self._renderer:  # Can be None during env reload.
      ...
      # print(self._camera.settings)
      # print(self._camera.transform)

      def construct_camera_matrix(lookat, distance, azimuth_degrees, elevation_degrees):
        azimuth = np.deg2rad(azimuth_degrees)
        elevation = np.deg2rad(elevation_degrees)
        forward = np.array([np.cos(azimuth) * np.cos(elevation), np.sin(azimuth) * np.cos(elevation), np.sin(elevation)])
        forward /= np.linalg.norm(forward)
        camera_position = lookat - forward * distance
        
        camera_matrix = np.eye(4)
        camera_matrix[0:3, 0] = np.cross(forward, [0, 0, 1])
        camera_matrix[0:3, 0] /= np.linalg.norm(camera_matrix[0:3, 0])
        camera_matrix[0:3, 1] = np.cross(camera_matrix[0:3, 0], forward)
        camera_matrix[0:3, 2] = forward
        camera_matrix[0:3, 3] = camera_position
        return camera_matrix
      settings = self._camera.settings
      camera_matrix = construct_camera_matrix(settings.lookat, settings.distance, settings.azimuth, settings. elevation)
      # print(list(camera_matrix.flatten()))
      camera_path = {"matrix" : list(camera_matrix.flatten('F')), 
                     "fov" : 80, "aspect": 1.6711229946524064}
      # if not self._pause_subject.value:
      #   print(camera_path, ',')
